...
def main():
    ...
    # Build tile list — mercantile.tiles expects lon/lat (EPSG:4326), so
    # reproject the source bounds whenever the source CRS is anything else.
    if _SRC_CRS is None:
        raise RuntimeError(f"{args.input} has no CRS — cannot tile.")
    if _SRC_CRS.to_epsg() == 4326:
        ll, bb, rr, tt = src_bounds.left, src_bounds.bottom, src_bounds.right, src_bounds.top
    else:
        ll, bb, rr, tt = transform_bounds(_SRC_CRS, "EPSG:4326", *src_bounds, densify_pts=21)

    all_tiles = []
    for z in range(args.zoom_min, args.zoom_max + 1):
        tiles = list(mercantile.tiles(ll, bb, rr, tt, zooms=z))
